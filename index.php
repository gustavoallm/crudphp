    ...
    <table class="table shadow-1g mt-3 table-hover text-white text-center" style="background-color:#333; border: 1px solid white">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <?php 
                    $cidade = $conexao->prepare("SELECT cidade FROM endereco where contato_id = :contato_id limit 1");
                    $cidade->bindParam(':contato_id', $contato_id , PDO::PARAM_STR);
                    $executa = $cidade->execute();
                    while($r_cidade = $cidade->fetch(PDO::FETCH_ASSOC))
                    { ?>
                        <a type="text" style="color: chartreuse; cursor: not-allowed;" class="btn btn-link text-decoration-none">
                ...
                <td>
                <?php 
                    $email = $conexao->prepare("SELECT email FROM email where contato_id = :contato_id limit 1");
